...
export default function App() {
  ...
  const createSender = async () => {
    ...
    // save this new KeyPair into this state variable (2)
    setSenderKeypair(senderKeypair);

    // request airdrop into this new account (3)
  ...
  const connectWallet = async () => {
    ...
    if (solana) {
      try {
        // connect to phantom wallet and return response which includes the wallet public key (5)
        const response = await solana.connect();
        console.log('wallet account ', response.publicKey.toString());
        // save the public key of the phantom wallet to the state variable (6)
  ...
  const transferSol = async () => {    
    ...
    // create a new transaction for the transfer (7)
    ...
    // send and confirm the transaction (9)
